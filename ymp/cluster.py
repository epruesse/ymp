...
class Slurm(ClusterMS):
    ...
    states = {
        ...
        'CONFIGURING': 'running',  # resources allocated, still waiting (e.g. booting node)
        'COMPLETING': 'running',  # some processes still active
        'DEADLINE': 'failed',  # job ran into deadline and was killed
        'FAILED': 'failed',  # job exited with code <> 0 or other failure condition
        'NODE_FAIL': 'failed',  # job terminated due to node(s) failed
        'PENDING': 'running',  # job waiting for resources
        'PREEMPTED': 'failed',  # job terminated to make room for important stuff
        'RUNNING': 'running',  # job has allocation and is hopefully working on sth
        'RESIZING': 'running',  # job is about to change size
        'SUSPENDED': 'running',  # job is paused
        'TIMEOUT': 'failed', # job reached time limit
        # questionable states:
        'SPECIAL_EXIT': 'running', # job failed but flagged into "special_exit"
        'REVOKED': 'running'  # job removed due to other cluster starting it
        # Both of the above technically mean that the job isn't "done" yet, running
        # either on another cluster or on hold for a re-run after initial failure.
    }
    ...
    def status(jobid):
        ...
        Anectotal benchmarking shows 200ms per invocation, half used by Python
...
if __name__ == "__main__":
    if len(sys.argv) < 3:
        ...
        Supported commands:
         - status <jobid>
           Returns running|success|failed status for jobid to be used by snakemake.
